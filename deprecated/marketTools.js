...
-moz-transform-origin: 0 0;
        ...
        `;
    saveVal("UIScale", scale.toString());
}

function darkModeFunc() {
    if (blackStyleCheckbox.checked) {
        saveVal("darkMode", "true")
        injectStyle.innerHTML = socialCSS
    } else {
        saveVal("darkMode", "false")
...
window.changeVol = function(v, m) {
    if (m == "music") {
        ...
        saveVal("UIVol", v.toString())
        window.soundVol2 = parseFloat(v)
    }
}

var pingHolder = document.createElement("div")
document.getElementById("aHolder").appendChild(pingHolder)
pingHolder.outerHTML = `<div id="pingHolder" style="color: rgb(255, 255, 255); position: absolute; bottom: 20px; font-size: 50px; left: 25px;">Loading</div>`

function showPingFunc() {
    let pingHolder = document.getElementById("pingHolder")
    if (!pingStyleCheckbox.checked) {
        document.getElementById("aContainer").style = ""
        pingHolder.style.display = "none"
        saveVal("showPing", "false");
    } else {
        document.getElementById("aContainer").style = "position:absolute; bottom: 0px; right: 13px"
        pingHolder.style = "color: rgb(255, 255, 255); position: absolute; bottom: 20px; font-size: 50px; left: 25px;"
        saveVal("showPing", "true");
    };
};

function showEstValFunc() {
    if (estValStyleCheckbox.checked) {
        st.innerHTML = `
.marketPrice {
top: 178px !important;
}`
        saveVal("showEstVal", "true")
    } else {
        st.innerHTML = `
.estVal2 {
display: none;
}`
        saveVal("showEstVal", "false");
    }
}
...
toolsHolder.innerHTML = `
        ...
        class="slider"></span></label><span id="blackStyleSliderText" style="">Zeno Theme</span>
</div>
<div id="pingStyleSwitch"><label class="switch"><input type="checkbox" id="pingStyleCheckbox"><span
        ...
        <div><span id="UIScaleText">UI Scale</span><input type="number" class="sliderVal" id="UIScaleInput" min="0.75" max="1" value="${getSavedVal("UIScale")}" onkeypress="document.getElementById('UIScaleSlider').value = this.value; changeUIScale(this.value);" style="margin-right: 0px;border-width:0px">
            <div class="slidecontainer">
            <input type="range" id="UIScaleSlider" min="0.75" max="1" step="0.05" value="${getSavedVal("UIScale")}" class="sliderM" oninput="document.getElementById('UIScaleInput').value = this.value; changeUIScale(this.value);"></div></div>
        <div><span id="MusicVolText">Music Volume</span><input type="number" class="sliderVal" id="MusicVolInput" min="0" max="1" value="${getSavedVal("musicVol")}" onkeypress="document.getElementById('MusicVolSlider').value = this.value; changeVol(this.value, 'music');" style="margin-right: 0px;border-width:0px">
            <div class="slidecontainer">
            <input type="range" id="MusicVolSlider" min="0" max="1" step="0.1" value="${getSavedVal("musicVol")}" class="sliderM" oninput="document.getElementById('MusicVolInput').value = this.value; changeVol(this.value, 'music');"></div></div>
        <div><span id="UIVolText">UI Volume</span><input type="number" class="sliderVal" id="UIVolInput" min="0" max="1" value="${getSavedVal("UIVol")}" onkeypress="document.getElementById('UIVolSlider').value = this.value; changeVol(this.value);" style="margin-right: 0px;border-width:0px">
            <div class="slidecontainer">
            <input type="range" id="UIVolSlider" min="0" max="1" step="0.1" value="${getSavedVal("UIVol")}" class="sliderM" oninput="document.getElementById('UIVolInput').value = this.value; changeVol(this.value);"></div></div>
        <span id="pingHolder2" style="text-align: center; display: block; margin-top: 20px; font-size: 40px;">Ping: Loading</span><br>
        </div>
</div>
        `;
changeUIScale(getSavedVal("UIScale"));
...
blackStyleCheckbox.checked = stringToBool(getSavedVal("darkMode"));
darkModeFunc();
pingStyleCheckbox.checked = stringToBool(getSavedVal("showPing"));
showPingFunc();
...
window.ws = new WebSocket('wss://krunker_social.krunker.io/ws');

var pingEncoded = window.msgpack.serialize(['po']).buffer;
...
window.ws.onmessage = async function(event) {
    ...
    var data = window.msgpack.deserialize(arrayBuffer);
...
window.skins = eval(((document.body.innerHTML).split(".exports.skins="))[1].split("]")[0] + "]");
window.avgPrices = [];
